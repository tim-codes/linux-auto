...
# edit fish config
set FISH_SOURCE_DIR "~/dev/linux-auto/fish"
alias cf="code $FISH_SOURCE_DIR/$1.fish"
alias ff="fleet $FISH_SOURCE_DIR/$1.fish"
alias vf="vim $FISH_SOURCE_DIR/$1.fish"

# reload fish config
function rf
  find $HOME/.config/fish -maxdepth 1 -name '*.fish' -not -name 'config.fish' -exec rm {} \;
  cp -i $HOME/dev/linux-auto/fish/*.fish $HOME/.config/fish/
  wait
  for file in (ls $HOME/.config/fish/*.fish)
    sed -i '1s/^/# ~~~~ WARNING: DO NOT EDIT THIS FILE ~~~~ #\n/' $file
  end
  source $fish_conf
end
